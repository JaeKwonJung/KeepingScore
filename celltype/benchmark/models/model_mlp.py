...
train_emb = torch.load("../../embedding/train_embedding.pt",  weights_only=True)
val_emb = torch.load("../../embedding/val_embedding.pt",  weights_only=True)
test_emb = torch.load("../../embedding/test_embedding.pt",  weights_only=True)
...
### Init model
MODEL = "mlp"
CHECKPOINT_PATH = os.path.join(f'../checkpoints/mlp/run{args.run_id}', MODEL)
LOGS_PATH = os.path.join(f'../logs/mlp/run{args.run_id}', MODEL)
    ...
    trainer_kwargs={
        ...
        'callbacks': [
            ...
            ModelCheckpoint(filename='val_loss_{epoch}_{val_loss:.3f}', monitor='val_loss', mode='min',
                            every_n_epochs=1, save_top_k=2),
            EarlyStopping(monitor="val_f1_macro", patience=20, mode="max", min_delta=1e-4, verbose=True),
        ],
    }
)
...
# After plotting
plt.savefig("mlp_lr_find_curve.png")
plt.close()
